...
function fileTypeCheck(
  ...
  const fileFormatSearch = (
    formatExtensionList: string[],
    fileExtensionValue
  ) =>
  ...
  const fileFormatResult: any = fileTypeList.find(({ formatExtensions }) =>
...
function isImported(fileArray: string[]): object {
  return fileArray.map((item: any) => {
    item.importedBy = fileArray
      .filter((file: any) =>
        file.imports.some((str: string) =>
          RegExp(item.fileName, "gi").test(str)
        )
      )
...
export default findUnusedImports;
